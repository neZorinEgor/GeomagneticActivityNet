...
#### Проблема единого `backbone`

Изначально была реализована архитектура с одной RNN-сетью и двумя головами (один экстрактор признаков -> отдельные головы для Dst и AE). Однако такой подход показал низкую эффективность: одна из голов предсказывала значительно хуже.

#### Причина: физическая разница индексов
| Характеристика | Dst (кольцевой ток) | AE (авроральный электроджет) |
|----------------|---------------------|------------------------------|
| **Динамика** | Медленная, инерционная | Быстрая, хаотичная |
| **Временная память** | Долгая (часы–дни) | Короткая (минуты–часы) |
| **Распределение** | Близко к нормальному | Сильно скошено влево |
| **Типичный диапазон** | -50…+50 нТл (спокойное время) | 0…500 нТл (спокойное время) |

Одному LSTM невозможно одновременно оптимально извлекать признаки для двух столь разных по своей природе индексов. Возникает конфликт градиентов, при котором модель вынуждена искать компромисс, и один из индексов (в нашем случае AE) "страдает"

#### Решение: раздельные ветки

```txt
Вход (14 признаков, 168 часов)
    │
    ├──► Dst_Attention (2 heads) → Dst_LSTM (64, 2 слоя) → Dst_Head → Dst
    │
    └──► AE_Attention  (7 heads) → AE_LSTM  (256, 5 слоёв) → AE_Head  → AE
```

Разделение позволило:
1. Каждой ветке специализироваться на своей задаче без взаимных помех
2. Достичь Dst RMSE 4.6–9.5 нТл и AE RMSE 68–122 нТл
3. Обучать модель стабильно, без конфликта градиентов
...
|---------|--------|---------------|------------|
| Train / Validation | 1996 – 2018 | ~210,000 | Обучение (90%) и валидация (10%) |
...
На графиках представлено сравнение предсказанных (оранжевый) и фактических (синий) значений индексов `Dst` и `AE` за 2019 и 2025 годы. (модель обучалась на данны с 1996 по 2018)
